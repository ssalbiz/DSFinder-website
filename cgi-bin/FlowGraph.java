 ...
 * for analyzing a method.  It consists of the basic blocks of a method.
 * <p>
 * 
 *
 * @see MethodEditor
...
public class FlowGraph extends Graph {
  ...
  /**
   * Returns the maximum loop depth (also the maximum loop height) in
  ...
  /**
   * Sets up the control flow graph.  Computes the dominators and the 
   * dominance frontier, cleans up the tree, works with the loops, 
   * inserts stores to aid copy and constant propagation as well as
   * code generation.
   */  
   ...
   * The loop tree represents the nesting of the loops in a method.
   * The procedure is at the root of the loop tree.  Nested loops are
   * represented by a parent and child relationship.
   */
   ...
   *
   * @param labelPos
   *        A mapping between the Labels in the code that start a 
  ...
  private void buildBlocks(Map labelPos) {
	...
	if (label.startsBlock()) {
	  ...
	  Block nextBlock = (Block) getNode(label);
	  ...
	  if (lastInst != null && lastInst.isJsr()) {
	    Block target = (Block) getNode(lastInst.operand());
  ...
  /**
   * Build the trees for the blocks, construct subroutines and
   * add edges in the flow graph.
   *
   * There is a edge from the source block to the init block,
   * to the entry of each subroutine and to the catch block
   * of each exception handler.
   *
   * After building trees for all nodes reachable from the source,
   * blocks with null trees are removed since they are unreachable.
   *
   * Edges are added to the sink block from each node ending in a
   * return, a throw, or a ret.
   *
   * In addition, an edge is added to the sink block from each node
   * ending in unconditional branch to an ancestor.  These edges are
   * used to allow the post dominator tree to be contructed in the
   * presence of loops.
   *
   * @param firstBlock
   *        The first block of code in this method.
   * @param labelPos
   *        A mapping between Labels and their instruction number 
  ...
  private void buildTrees(Block firstBlock, Map labelPos) {
    ...
    HashMap catchBodies = new HashMap(method.tryCatches().size()*2+1);
    ...
    while (tryCatches.hasNext()) {
      ...
      Block catchBody = (Block) getNode(tc.handler());
   ...
   *         Y
   *
   * We can't do this for reference types since we may
   * loose type information.  Consider:
   *
   * class A {}
  ...
  private void insertConditionalStores() {
	...
	if (target != null) {
	  ...
	  if (! left.type().isReference() &&
	    ...
	    // that stores the result of the expression into the local
	    // variable.

	    if (! (left instanceof LeafExpr)) {
	      LocalVariable v = method.newLocal(left.type());
	      LocalExpr tmp = new LocalExpr(v.index(),
					    left.type());
	      Expr copy = (Expr) left.clone();
	      copy.setDef(null);
	      left.replaceWith(new StoreExpr(tmp,
					     copy, left.type()));
	      left = tmp;
	    }
	    
	    if (! (right instanceof LeafExpr)) {
	      LocalVariable v = method.newLocal(right.type());
	      LocalExpr tmp = new LocalExpr(v.index(),
					    right.type());
	      Expr copy = (Expr) right.clone();
	      copy.setDef(null);
	      right.replaceWith(new StoreExpr(tmp,
					      copy, right.type()));
	      right = tmp;
	    }
	    ...
	    if (left instanceof LocalExpr) {
	      ...
	      Expr copy = (Expr) left.clone();
	      copy.setDef(null);
	      Stmt insert = new ExprStmt(new 
					 StoreExpr(tmp, copy, right.type()));
	...
	if (target != null) {
	  ...
	  if (! left.type().isReference()) {
	    ...
	    // variable and replace left with an assignment from left
	    ...
	    if (! (left instanceof LeafExpr)) {
	      ...
	      Expr copy = (Expr) left.clone();
	      copy.setDef(null);
	      left.replaceWith(new StoreExpr(tmp, copy,
					     left.type()));
	      left = tmp;
	    }
	    
	    // Value of the right hand side.  0 if left is an integer,
	    ...
	    if (left instanceof LocalExpr) {
	      // Prepend the target block with an assignment from the
	      // value of the right hand side to the left expression.

	      LocalExpr copy = (LocalExpr) left.clone();
	      copy.setDef(null);
	      Stmt insert = new ExprStmt(new 
					 StoreExpr(copy, new 
						   ConstantExpr(value, type),
						   left.type()));
	...
	if (! (index instanceof LeafExpr)) {
	  ...
	  LocalVariable v = method.newLocal(index.type());
	  LocalExpr tmp = new LocalExpr(v.index(), index.type());
	...
	if (index instanceof LocalExpr) {
	  ...
	  for (int i = 0; i < targets.length; i++) {
	    ...
	    // Skip targets that can be branched to in multiple places
	    ...
	    copy.setDef(null);
	    Stmt insert = new
	      ExprStmt(new StoreExpr(copy, new 
				     ConstantExpr(new Integer(values[i]),
						  index.type()), 
				     index.type()));
  ...
  /**
   * Insters copy statements into blocks whose successors lie in a
   * protected region.
   *
   * @param block
   *        A block we are considering.
   * @param tryPreds
   *        All blocks whose successor blocks lie in protected 
   *        regions.
   * @param defs
   *        Stores the expressions (LocalExprs) that define a given 
   *        local variable (index into array) in block.  Basically, it
   *        contains the LocalExpr that defines each local variable at
   *        a given block.
   */
  ...
  /**
   * Removing unreachable Blocks means that there are Labels that are
   * no longer label valid blocks (e.g. start basic blocks) in the
   * CFG.  However, we still want the Labels to point to something
   * meaningful.  So, we hoist them out of CFG and place them into the
   * init block as LabelStmts.  
   */
  private void saveLabels()
  {
    ...
    Iterator iter = method.code().listIterator();
  ...
  /**
   * Returns a new <tt>Block</tt> with the next available
   * <tt>Label</tt>.
   */
  ...
  /**
   * Creates a new Block starting with the specified Label.  The Block is
   * added to this FlowGraph using its label as its key.
   *
   * @param label
   ...
   * splitIrreducibleLoops first to split reducible loop headers
   * from irreducible loops.  This method is based on the analyze_loops
   * algorithm in:
   * 
  ...
  private void setBlockTypes() {
    ...
    while (iter.hasPrevious()) {
      ...
      Set body = new HashSet();  // The body of a loop
	...
	while (e.hasNext()) {
	  ...
	  Block z = (Block) blocks.get(uf.find(yn));  // loop header of y
	  
	  if (! isAncestorToDescendent(w, z)) {
	    // If a block in the loop is not a descendent of the loop
	    // header, then there must be another entry path into the
	    // loop.  Thus, the loop (and its header) are IRREDUCIBLE.
  ...
  /**
   * Ensure that no reducible back edge shares a destination with
   * a irreducible back edge by splitting reducible loop headers from
  ...
  /**
   * Ensure that no reducible back edge shares a destination with
   * another reducible back edge by splitting reducible loop headers.
   * This makes loop nesting easier to detect since each loop has a
   * unique header.
   */
  private void splitReducibleLoops() {
    ...
    while (iter.hasNext()) {
      ...
      Set edges = new HashSet();   // reducible back edges
	...
	if (min == null || vn < preOrderIndex(min)) {
      ...
      tree.addInstruction(
			  new Instruction(Opcode.opcx_goto, w.label()));
  ...
  /**
   * Loop peeling is a process by which the first iteration of a loop
   * is duplicated in the control flow graph.  An code that has side
   * effects (such as throwing an exception) will be tested in the
   * first iteration.  This allows us to make assumptions about the
   * code in the second iteration.
   *
   * To detect loop nesting more easily we require that each loop header
   * have at most one incoming back edge.
   *
   * For each loop, peel up to the last exit if:
   *     1. There is more than one exit, or,
   *     2. The last exit has a successor in the loop body (not the 
   *        header).  
   */
  private void peelLoops(int level) {
    ...
    for (int i = 0; i < loops.size(); i++) {
      ...
      levels[i] = loop.level;
      maxLoopDepth = (loop.level > maxLoopDepth ? loop.level : maxLoopDepth);
    }
  ...
  LOOPS:
    ...
    for (int i = 0; i < peel.size(); i++) {
      ...
      Collection outerLoop = (Collection)
	loops.get(outerLoopIndex.intValue());
	...
	while (e.hasNext()) {
	  ...
	  while (succs.hasNext()) {
	    ...
	    if (! loop.contains(succ)) {
	      // If the successor of one of the blocks in the loop is
  ...
  /**
   * Returns the <tt>Subroutine</tt> whose entry block is labeled by a 
   * given <tt>Label</tt>.
   */
  ...
  /**
   * Set the entry in the mapping between subroutine entry 
   * <tt>Block</tt>s and the <tt>Subroutine</tt>s that they begin.
   * It also sets the <tt>Subroutine</tt>'s entry block.
   *
   * @param sub
  ...
  /**
   * Returns all of the <tt>Subroutine</tt>s in the method modeled by 
   * this <tt>FlowGraph</tt>.
   */
  ...
  {
    String dateString = java.text.DateFormat.getDateInstance().format(new Date());
    out.println("Print " + ++file + " at " +
		dateString + " " +
  ...
  public void printGraph() {
    try {
      PrintStream out = 
	new PrintStream(new FileOutputStream(method.name() + "." + 
					     next++ + ".dot"));
      printGraph(out);
  ...
  /**
   * Creates a graphical description of the CFG in the dot language. The
   * name of the generated file is the name of the method modeled by 
   * this CFG followed by a number and the ".dot" postfix.  For more 
   * information about dot and tools that use it see:
   * <p align=center>
  ...
  /** 
   * Removes the critical edges (edges from a block with more
   * than one successor to a block with more than one predecessor)
   * from the graph.  Critical edges can screw up code motion.  
   * <p>
   * For code generation, the block must be inserted after the
   * predecessor if the successor is the default target.  Throw
   * successors and predecessors are copied from the successor block.
   */
  private void removeCriticalEdges()
  {
    ...
    while (e.hasNext()) {
      ...
      if (hasEdge(v, w)) {
	if (FlowGraph.DEBUG) {
	  System.out.println("removing critical edge from " +
			     v + " to " + w);
	}
   ...
   * destination block.  The new block consists of a goto to the
   * destination block.  However, later optimizations may move code
   * from the destination block into the new block.  Thus, if the
   * destination block is a proteced block, the new block must also be
   * a protected block.
   */
  ...
  /**
   * Finds any blocks in the CFG that are both the entry block of a
   * subroutine and the return target of (another) subroutine.
   * <p>
   * The Subroutine's in the cfg are examined.  If a block is 
   * encountered that is both a subroutine entry block and the target
   * of a subroutine return, then we have to make two new blocks: a
   * new target block and a new entry block.  The edges have to be
  ...
  ENTRIES:
	...
	while (paths.hasNext()) {
	  ...
	  // block of entrySub, then note the caller of returnSub as the
	  // returnSubCaller.
	  if (block == path[1]) {
	...
	System.out.println("" + block +
      ...
      tree.addInstruction(
			  new Instruction(Opcode.opcx_goto, block.label()));
  ...
  /**
   * Builds the expressions trees for the "special" blocks (source, sink,
   * and init blocks).  Empty expressions trees are built for the source
   * and sink blocks.  The init block's expression tree contains code
   * that initializes the method's parameters (represented as local
   * variables).
   * <p>
   *
   * @param catchBodies
  ...
  /**
   * If a block may throws an exception (i.e. it is in a protected 
   * region), there must be an edge in the control flow graph from that
   * block to the block that begins the exception handler.  This method
   * adds that edge.
   * <p>
   * We iterate over all of the Handler objects created for this 
   * FlowGraph.  If the block of interest lies in the protected region
   * of the Handler, make note of this fact and add an edge between the
   * block and the first block of the exception handler.  Generate the
   * expression tree for the exception handler, if necessary.
   * <p>
   * Recursively call addHandlerEdges for the exception handler to
   * accomodate exception handlers within exception handlers.
   *
   * @param block
			       ...
			       Map catchBodies, Map labelPos, Subroutine sub)
  {
	...
	if (catchBody.tree() == null) {
	  ...
	  buildTreeForBlock(catchBody, s, sub,
			    labelPos, catchBodies);
	}
  ...
  /**
   * Dave sez: Builds the expression tree for a basic block and all
   * blocks reachable from that block.  Basically, the block's code
   * (Instructions and Labels) are iterated over.  The instructions
   * are added to the tree with calls to Tree#addInstruction.  If an
   * instruction invovles a change of control flow (e.g. jsr, jump,
   * switch), add an edge in the control flow graph between the
   * appropriate blocks.  After all that is done, call addHandlerEdges
   * to add edges between blocks that may throw exceptions and the
   * blocks that handle those exceptions
   *
   * Nate sez: Visit a block other than source or catch.  Since blocks
   * are visited depth-first, one predecessor was already visited, get
   * the operand stack state at the end of the predecessor block and
   * use it as the initial operand stack state for this block.  We
   * assume the class file passed verification so which predecessor
  ...
  private void buildTreeForBlock(Block block, OperandStack stack,
				 Subroutine sub, Map labelPos, Map catchBodies)
  {
    if (block.tree() != null) {
      return;
    }
    ...
    ListIterator iter = method.code().listIterator(start.intValue()+1);
  ...
  CODE:
	...
	if (inst.opcodeClass() == Opcode.opcx_astore) {
	  ...
	  buildTreeForBlock(target, tree.stack(), nextSub,
			    labelPos, catchBodies);
	  ...
	  buildTreeForBlock(target, tree.stack(), sub,
			    labelPos, catchBodies);
	  ...
	  addEdge(block, next);
	  buildTreeForBlock(next, tree.stack(), sub,
			    labelPos, catchBodies);
	...
	if (label.startsBlock()) {
	  tree.addInstruction(
			      new Instruction(Opcode.opcx_goto, label));

	  Block next = (Block) getNode(label);

	  Assert.isTrue(next != null,
			"Block for " + label + " not found");
   ...
   * followed by their false branch and, where possible, that blocks that
   * end in an unconditional jump are followed by the block that is the
   * target of the unconditional branch.
   * <p>
  ...
  public List trace() {
    ...
    Assert.isTrue(trace.size() == size()-2,
		  "trace contains " + trace.size() + " " + trace +
		  " blocks, not " + (size()-2) + " " + nodes());
    return trace;
  }
  ...
  public void commit() {
	...
	if (handler.protectedBlocks().contains(block)) {
	  ...
	  TryCatch tc = new TryCatch(begin.label(),
				     block.label(), catchBlock.label(), type);
  ...
  /**
   * Returns the "Enter" block of this CFG.  That is, the block through
   * which all paths enter.
   */
  public Block source()
  {
    return srcBlock;
  ...
  /**
   * Returns the iterated dominance frontier (DF+) for a given set of
   * blocks.
   * <p>
   * The iterated dominance frontier for a set of nodes is defined to be
   * the union of the dominance frontiers of all the nodes in the set.
   * <p>
   * The iterated dominance frontier is particularly useful because the
   * DF+ of an assignment node for a variable (expression) specifies the 
   * nodes at which phi-functions (PHI-functions) need to be inserted.
   *
   * @param blocks
   *        The
   * @param reverse
   *        Do we find the reverse (i.e. postdominance) dominance frontier.
   *
   ...
   *         case there is only one root, so the Collection only contains
   *         the source block.
   */
  public Collection roots()
  {
  ...
  /** 
   * Returns A Map mapping the first block in an exception handler to
   * its <tt>Handler</tt> object.
   *
   * @see Handler */
  ...
  /**
   * Returns the<tt>Block</tt>s in this CFG that begin exception
   * handlers.
   */
  ...
  /**
   * Returns the depth of the loop in which a block is contained.  The 
   * block must be contained in a loop.  The procedure has depth 0.
   * A loop (while, for, etc.) at the procedure level has depth 1.
   * Depth increases as loops are nested.
   *
   * @param block
  ...
  /**
   * Returns the level of the loop containing a given block.  The 
   * innermost loops have level 0.  The level increases as you go
   * outward to higher loop nestings.  For any given loop, the level
   ...
   * The loop header is the block that dominates all of the blocks
   * in the loop.
   */
  ...
  /**
   * A LoopNode is a node in the loop tree.  The loop tree is represents
   * the nesting of loops in the method being modeled in this CFG.
   *
   * @see loopDepth
   * @see loopLevel
   */
  ...
  /**
   * Returns a brief textual description of this <tt>FlowGraph</tt>,
   * namely the name of the method it represents.
   */
  public String toString() {
